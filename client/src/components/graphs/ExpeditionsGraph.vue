...
export default {
  ...
  methods: {
    ...
    async handleBarClick(d) {
      ...
      content.innerHTML = `${'Individuals from ' + d[0]}<br>${createTableFromJson(response)}`;
    ...
    buildExpeditionGraph() {
      ...
      d3.select(this.$refs.expeditionsGraph).select("svg").remove();
      ...
      let svg = d3
        .select(this.$refs.expeditionsGraph)
        .append("svg")
      ...
      let x = d3
        .scaleBand()
        .range([0, width])
        .domain(this.deathsByExpedition.map((d) => d[0]))
        .padding(0.2);
      svg
        .append("g")
      ...
      let y = d3
        .scaleLinear()
        .domain([0, d3.max(this.deathsByExpedition, (d) => d[1])])
        .range([height, 0]);
